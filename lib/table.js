...
function serializedValuesEqual(a,b)
{
    ...
    if (a instanceof Array) {
        ...
        for (var i = 0; i < a.length; ++i) {
            if (!serializedValuesEqual(a[i],b[i])) return false;
        }
        return true;
    }
...
module.exports.prototype.remove = function(id)
{
    ...
    if (row) {
        ...
        if (row.state === RowState.DELETING) return false;
        _pendingChanges.push([ "D", this.id, id ]);
    }
    return false;
};
...
module.exports.getAll = function(opts)
{
    opts = opts || {};
    opts.idSetter = opts.idSetter || defaultIdSetter;
...
module.exports.update = function(update,opts)
{
    ...
    if (!id) {
        throw new Error('Object to be updted must have a non null/empty Id');
    }
    ...
    if (row) {
        ...
        for (var key in update) {
            ...
            value = value ? serializeValue(value) : null;
            ...
            if (!serializedValuesEqual(a[i],b[j])) {
                ...
                if (num[i][j] > maxlen) {
                    ...
                    if (lastSubsBegin === thisSubsBegin) {
                        ...
                        sequence = [];
                        for (var k = 0; k < (i+1) - lastSubsBegin; ++k) {
                            sequence.push(a[lastSubsBegin+k]);
                        }
                    }
                }
            }
