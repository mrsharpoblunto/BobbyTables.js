...
function load(datastore,json)
{
    ...
    for (var key in json.tables) {
        if (json.tables.hasOwnProperty(key)) {
            var table = new Table(datastore,key);
            table._load(json.tables[key]);
...
module.exports.prototype.save = function() {
    ...
    for (var key in this._tables) {
       if (this._tables.hasOwnProperty(key)) {
          json.tables[key] = this._tables[key]._save();
       }
    }
    return json;
};
...
module.exports.prototype.pull = function(callback) {
    for (var key in this._tables) {
        ...
        if (this._tables[key]._hasPendingChanges()) {
            return callback(new Error('Unable to pull in remote changes to datastore with id ' + this.id + ' as it has local changes pending'),null);
        }
    }
    ...
    var request;
    if (this.rev === 0) {
        request = new this._manager._ApiRequest('POST','get_snapshot',this._manager.apiToken,this._manager._opts,this._manager._opts);
        ...
        request = new this._manager._ApiRequest('POST','get_deltas',this._manager.apiToken,this._manager._opts,this._manager._opts);
...
module.exports.prototype._applyChanges = function(body)
{
    ...
    for (var i = 0;i < body.deltas.length; ++i) {
        ...
        for (var j = 0;j < delta.changes.length;++j) {
            ...
            var table = this._tables[tid];
            if (!table) {
                table = new Table(this,tid);
                this._tables[tid] = table;
            }
...
module.exports.prototype.awaitPull = function(callback)
{
    ...
    var args = { cursors: { } };
    args.cursors[this.handle] = this.rev;
    var self = this;

    var request= new this._manager._ApiRequest('GET','await?get_deltas=' + encodeURIComponent(JSON.stringify(args)),this._manager._opts);
...
module.exports.prototype.push = function(callback) {
    ...
    var self = this;
    var request = new this._manager._ApiRequest('POST','put_delta',this._manager.apiToken,this._manager._opts);
    request
    ...
    request.getResponse(function(err,response) {
        ...
        try 
        {
            ...
            for (var key in self._tables) {
                if (self._tables.hasOwnProperty(key)) {
